...
function AppMain({ user, onLogout, dark, setDark, T, isRecovery, onRecoveryHandled, soundOn, toggleSound }) {
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => requestAnimationFrame(() => {
      ...
      if (el) {
        // Manual scroll to account for sticky header
        const stickyH = (calStickyRef.current?.offsetHeight || 0) + 60 + 8; // sticky + header + gap
        const top = el.getBoundingClientRect().top + window.scrollY - stickyH;
        window.scrollTo({ top, behavior: calInitialScroll.current ? "smooth" : "auto" });
      }
  ...
  return (
    <div className={dark ? "dark" : ""} style={{ minHeight: "100vh", background: T.bg, fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', system-ui, sans-serif", letterSpacing: "-0.015em", position: "relative", paddingRight: wideEnough ? (showCanvas ? `${canvasWidth}px` : "48px") : 0, boxSizing: "border-box" }}>
      ...
      <main id="main-content" style={{ maxWidth: "520px", margin: "0 auto", padding: "77px 20px 190px" }}>
        ...
        <div ref={calStickyRef} style={isCalendarMode ? { position: "sticky", top: "40px", zIndex: 30, background: T.bg, marginLeft: "-20px", marginRight: "-20px", paddingLeft: "20px", paddingRight: "20px", paddingTop: "24px", paddingBottom: "8px", borderBottom: `0.5px solid ${T.border}` } : undefined}>
        ...
        {isCalendarMode && <>
          ...
          {overdueTasks.length > 0 && (
            <button onClick={() => setShowOverdueModal(true)}
              style={{ width: "100%", display: "flex", alignItems: "center", gap: "10px", padding: "12px 16px", borderRadius: "14px", border: `1px solid ${T.danger}30`, background: `${T.danger}0A`, cursor: "pointer", marginBottom: "4px", textAlign: "left" }}>
